...
if(!file.exists(spp_habs_from_api_file) | reload) {
  ...
  for(j in 1:n_chunks) { 
    ...
    if(!file.exists(chunk_file)) {
      ...
      spp_habs_chunk <- mc_get_from_api(spp_habs_url, spp_ids_chunk, 
...
hab_cats <- read_csv(spp_habs_from_api_file, col_types = cols(.default = 'c')) %>%
  select(habitat, code) %>%
  distinct() %>%
...
spp_systems_from_api <- read_csv(spp_sys_from_api_file,
                              col_types = cols(.default = 'c')) %>%
  mutate(iucn_sid = as.integer(iucn_sid),
         marine_system = as.logical(marine_system)) %>%
...
#   filter(include == TRUE | (is.na(include) & marine_system)) %>%
#   distinct()
...
iucn_spp_info <- read_csv(file.path(dir_o_anx, 'iucn',
...
if(!file.exists(spp_timeseries_file) | reload) {
  ...
  ptm <- proc.time()
  spp_past_df <- mc_get_from_api(spp_hist_url, sid_list, api_key, 
                                 delay = 1, cores = 16)
  message('elapsed time: ', (proc.time() - ptm)[3])
...
spp_current_file <- file.path(dir_data, 
