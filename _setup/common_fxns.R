...
### this is for clipping to globe, but allowing for segmented longitude
### cuts...
bbox_vert <- seq(-90, 90, .25)
bbox_df <- data.frame(
  long = c(rep(-180, times = length(bbox_vert)), 
           rep(180, times = length(bbox_vert)), -180),
  lat  = c(bbox_vert, desc(bbox_vert), -90))

bbox_segmented <- st_polygon(x = list(as.matrix(bbox_df))) %>%
  st_sfc() %>%
  st_set_crs(4326)

clip_to_globe_segmented <- function(x) {
  ### for SF features, transform to wgs84, clip to +-180 and +-90;
  ### add segmentation to allow for smoother transformations along
  ### +/- 180 line
  epsg <- st_crs(x)$epsg
  if(epsg != 4326 | is.na(epsg)) {
    message('Original EPSG = ', epsg, '; Proj4 = ', st_crs(x)$proj4string,
  ...
  if(x_bbox$xmin < -180 | x_bbox$xmax > +180 |
     x_bbox$ymin <  -90 | x_bbox$ymax >  +90) {
    message('Some bounds outside +-180 and +-90 - clipping')
    z <- st_intersection(x, bbox_segmented) %>%
      st_cast('MULTIPOLYGON')
    # z <- as(x, 'Spatial') %>%
    #   raster::crop(raster::extent(-180, 180, -90, 90)) %>%
    #   st_as_sf()
