...
dir_shp <- file.path(dir_M, 'git-annex/globalprep/_raw_data/iucn_spp/d2019')
...
if(reload == FALSE) {
  maps_already_rasterized <- list.files(file.path(dir_o_anx, 'spp_rasts_mol_2019'),
                                        pattern = '.csv') %>%
    str_extract('[0-9]+') %>%
    as.integer()
...
# zxcv <- list.files(file.path(dir_o_anx, 'spp_rasts_mol_2019'), 
#                    full.names = TRUE, 
...
if(nrow(maps_to_rasterize) == 0) { ### all maps accounted for as .csvs
  ...
  for(i in seq_along(shps)) {
    ...
    #   cat_msg('Replacing REPTILES with buffered turtle polygons!!!')
    ...
    #   # st_write(polys_match, file.path(dir_o_anx, 'reptiles_shp_buffered.shp'))
    #   # dbf <- foreign::read.dbf(file.path(dir_o_anx, 'reptiles_shp_buffered.dbf'))
    #   # foreign::write.dbf(dbf, file.path(dir_o_anx, 'reptiles_shp_buffered.dbf'))
    #   
    #   polys_match <- st_read(file.path(dir_o_anx, 'reptiles_shp_buffered.shp')) %>%
    ...
    # system.time({
      tmp <- parallel::mclapply(seq_along(spp_ids),
        ...
        ### This is a bunch of stuff to test the new clipping
        ### algorithm that provides for segmentation along
        ### clipped edges, so Mollweide can round the corners.
        # test <- st_polygon(x = list(
        #   matrix(c(-185, -185, 185, 185, -185,
        #            -40,    40,  70, -70,  -40), ncol = 2))) %>%
        #   st_sfc() %>%
        #   st_set_crs(4326)
        # test_clip <- test %>%
        #   clip_to_globe() %>%
        ...
        # rast_file <- file.path(dir_o_anx, 'spp_rasts_mol_2019',
        #                        sprintf('iucn_sid_%s_mol.tif', spp))
        ...
        write_csv(spp_present, file.path(dir_o_anx, 'spp_rasts_mol_2019',
                                         sprintf('iucn_sid_%s_mol.csv', spp)))
...
csvs <- list.files(file.path(dir_o_anx, 'spp_rasts_mol_2019')) %>%
  str_extract('[0-9]+')

x <- list.files(file.path(dir_o_anx, 'spp_rasts_mol_2019'),
                full.names = TRUE)
