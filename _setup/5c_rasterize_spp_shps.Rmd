...
if(reload == FALSE) {
  maps_already_rasterized <- list.files(file.path(dir_o_anx, 'spp_rasters_2019'),
                                        pattern = '.csv') %>%
...
# maps_to_rasterize <- maps_to_rasterize %>%
#   filter(str_detect(dbf_file, 'CORALS'))
# 
# asdf <- paste0('iucn_sid_', maps_to_rasterize$iucn_sid, '.csv')
# zxcv <- list.files(file.path(dir_o_anx, 'spp_rasters_2019'), 
#                    full.names = TRUE, 
  ...
  for(i in seq_along(shps)) {
    ...
    # system.time({
      tmp <- parallel::mclapply(seq_along(spp_ids),
        ...
        # rast_file <- file.path(dir_o_anx, sprintf('spp_rasters_2019/iucn_sid_%s.tif', spp))
        ...
        write_csv(spp_present, file.path(dir_o_anx, 'spp_rasters_2019',
...
csvs <- list.files(file.path(dir_o_anx, 'spp_rasters_2019')) %>%
  str_replace_all('iucn_sid_|.csv', '')

x <- list.files(file.path(dir_o_anx, 'spp_rasters_2019'),
                full.names = TRUE)
