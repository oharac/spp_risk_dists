...
reload <- TRUE
### unlink(spp_range_file)


### load area raster, convert to data frame
area_rast <- raster(file.path(dir_spatial, 'ocean_area_mol.tif'))
...
if(length(spp_ids) > 0) { ### any more spp to calculate?
  ...
  spp_range_list <- parallel::mclapply(seq_along(spp_ids), mc.cores = 24,
    ...
    spp_map <- read_csv(file.path(dir_o_anx, 'spp_rasts_mol_2019',
                                  sprintf('iucn_sid_%s_mol.csv', x)),
                        col_types = 'di')
...
spp_range <- read_csv(spp_range_file,
                      col_types = 'dd__')

# ocean_area_rast <- raster(file.path(dir_spatial, 'ocean_area_mol.tif'))
...
### raster for cell IDs
rast_cell_ids <- raster(file.path(dir_spatial, 'cell_id_mol.tif'))

### Make a dataframe of cell ID to MEOW for regional assessments... also 
### make a lookup of MEOW to region
meow_rgns_rast <- raster::raster(file.path(dir_spatial, 'meow_mol.tif'))
...
for(taxon in taxa) {
  ...
  taxon_risk_sum_file <- file.path(dir_o_anx, 'taxa_sums_mol_2019',
  ...
  if(!file.exists(taxon_risk_sum_file) | reload == TRUE) {
    ...
    taxon_cells_list <- parallel::mclapply(taxon_ids, mc.cores = 24,
        ...
        ### x <- taxon_ids[11]
        ### x <- 6336                            
        csv_file <- file.path(dir_o_anx, 'spp_rasts_mol_2019', 
                              sprintf('iucn_sid_%s_mol.csv', x))
