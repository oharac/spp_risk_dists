...
risk_by_cell <- read_csv(file.path(dir_data, 'risk_by_cell_by_taxa.csv'),
                         col_types = 'dcdd') %>%
  # filter(n_spp >= 5) %>% ### leave all in for this analysis
  mutate(mean_risk = 100 * mean_risk)


for(gp in risk_by_cell$taxa %>% unique()) {
  tmp_df <- risk_by_cell %>%
    filter(taxa == gp)
...
for(stressor_file in stressor_files) {  ### stressor_file <- stressor_files[3]
  ...
  ### For non-climate stressors, set NA values to zero
...
### use stressor_df and risk_by_cell from before
...
# mpa_age_check <- read_csv(file.path(dir_data, 'wdpa_i_iv_lookup.csv'), col_types = 'ddd') %>%
#   arrange(year) %>%
#   mutate(cum_area = cumsum(wdpa_yr_km2)) %>% ### 9486812 total by 2016; 
#   filter(cum_area >= max(cum_area) / 2) ### 2006
### So 2006 is the median year for protected area under IUCN I-IV...

mpa_new_df <- read_csv(file.path(dir_data, 'wdpa_i_iv_lookup.csv'), col_types = 'ddd') %>%
  left_join(read_csv(file.path(dir_data, 'ocean_area_lookup.csv'), col_types = 'dd')) %>%
  group_by(loiczid) %>%
  filter(year >= 2006) %>%
  summarize(mpa_new_pct = sum(wdpa_yr_km2) / first(ocean_area_km2))

eez_df <- read_csv(file.path(dir_data, 'eez_cells.csv'))

risk_v_stressor_df <- full_join(risk_by_cell, stressor_df, by = 'loiczid') %>%
  ...
  left_join(mpa_new_df, by = 'loiczid') %>%
  mutate(eez         = ifelse(is.na(eez), 0, eez),
         mpa_pct     = ifelse(is.na(mpa_pct), 0, mpa_pct),
         mpa_new_pct = ifelse(is.na(mpa_new_pct), 0, mpa_new_pct))
