---
title: 'Biodiversity risk maps - range-rarity weighting - Gall-Peters'
...
Create a set of maps of the distribution of biodiversity intactness - all species assessed and mapped by IUCN.  These maps are generated at 10 km^2^ resolution on a Gall-Peters projection.  All values are calculated based on range rarity, i.e. a cell's value depends on the status and relative proportion of range of each species present in the cell.  These maps will be generated using all available species:
...
cell_summary_file <- file.path(dir_o_anx, 'cell_summary_rr_weighted_gp.csv')
...
if(!file.exists(cell_summary_file) | reload == TRUE) {
  ...
  sum_files <- list.files(dir_taxa_summaries,
                          pattern = sprintf('cell_sum_rrweight_%s_gp.csv', api_version),
                          full.names = TRUE)
...
map_plot_file <- file.path(dir_git, 'ms_figures/fig1_map_mean_risk_rr_gp.png')


if(!file.exists(map_plot_file) | reload == TRUE) {
  
  mean_rast <- subs(rast_base, cell_summary, by = 'cell_id', which = 'mean_risk')
  writeRaster(mean_rast, file.path(dir_git, 'output', 'mean_rr_risk_raster_gp.tif'),
              overwrite = TRUE)
  
  ### mean_rast <- raster(file.path(dir_git, 'output', 'mean_rr_risk_raster_gp.tif'))
  ...
  #        fill  = 'Mean risk')
  # 
  # ggsave(filename = map_plot_file %>% str_replace('_rr_gp.png', '_ne_pac_rr_gp.png') %>%
  ...
  #        fill  = 'Mean risk')
  # 
  # ggsave(filename = map_plot_file %>% str_replace('_rr_gp.png', '_med_rr_gp.png') %>%
  #          str_replace('fig1', 'fig_SI'),
  #        width = 6, height = 4, units = 'in', dpi = 300)
  # 
  # x <- ggplot(mean_df) +
  ...
  #        fill  = 'Mean risk')
  # 
  # ggsave(filename = map_plot_file %>% str_replace('_rr_gp.png', '_mid_atl_rr_gp.png') %>%
  #          str_replace('fig1', 'fig_SI'),
  #        width = 6, height = 4, units = 'in', dpi = 300)
  # 
  # 
  ...
  #        fill  = 'Mean risk')
  # 
  # ggsave(filename = map_plot_file %>% str_replace('_rr_gp.png', '_se_asia_rr_gp.png') %>%
  #          str_replace('fig1', 'fig_SI'),
  #        width = 6, height = 4, units = 'in', dpi = 300)
...
![](ms_figures/fig1_map_mean_risk_rr_gp.png)
<!-- ![](ms_figures/fig_SI_map_mean_risk_ne_pac_rr_gp.png) -->
<!-- ![](ms_figures/fig_SI_map_mean_risk_med_rr_gp.png) -->
<!-- ![](ms_figures/fig_SI_map_mean_risk_mid_atl_rr_gp.png) -->
<!-- ![](ms_figures/fig_SI_map_mean_risk_se_asia_rr_gp.png) -->

``` {r var_risk_raster, eval = TRUE}

map_plot_file <- file.path(dir_git, 'ms_figures/fig_SI_map_var_risk_rr_gp.png')

if(!file.exists(map_plot_file) | reload == TRUE) {
  
  var_rast <- subs(rast_base, cell_summary, by = 'cell_id', which = 'var_risk')
  writeRaster(var_rast, file.path(dir_git, 'output', 'var_rr_risk_raster_gp.tif'),
              overwrite = TRUE)
...
![](ms_figures/fig_SI_map_var_risk_rr_gp.png)

``` {r n_spp_risk}

map_plot_file <- file.path(dir_git, 'ms_figures/fig_SI_map_sr_risk_rr_gp.png')

if(!file.exists(map_plot_file) | reload == TRUE) {
  
  n_spp_rast <- subs(rast_base, cell_summary, by = 'cell_id', which = 'sr_rr_risk')
  writeRaster(n_spp_rast, file.path(dir_git, 'output', 'sr_rr_risk_raster_gp.tif'),
              overwrite = TRUE)
...
![](ms_figures/fig_SI_map_sr_risk_rr_gp.png)

``` {r n_spp_threatened}

map_plot_file <- file.path(dir_git, 'ms_figures/fig2_map_pct_threatened_rr_gp.png')

if(!file.exists(map_plot_file) | reload == TRUE) {
  
  n_threat_rast <- subs(rast_base, cell_summary, by = 'cell_id', which = 'sr_rr_threatened')
  writeRaster(n_threat_rast, file.path(dir_git, 'output', 'sr_rr_threat_raster_gp.tif'),
              overwrite = TRUE)
  ### n_threat_rast <- raster(file.path(dir_git, 'output', 'sr_rr_threat_raster_gp.tif'))
  ...
  writeRaster(pct_threat_rast, file.path(dir_git, 'output', 'sr_rr_pct_threat_raster_gp.tif'),
              overwrite = TRUE)
  ### n_threat_rast <- raster(file.path(dir_git, 'output', 'sr_rr_pct_threat_raster_gp.tif'))
...
![](ms_figures/fig2_map_pct_threatened_rr_gp.png)

``` {r trend}

map_plot_file <- file.path(dir_git, 'ms_figures/fig_SI_map_mean_trend_rr_gp.png')

if(!file.exists(map_plot_file) | reload == TRUE) {
  
  trend_rast <- subs(rast_base, cell_summary, by = 'cell_id', which = 'mean_trend')
  writeRaster(trend_rast, file.path(dir_git, 'output', 'trend_rr_raster_gp.tif'),
              overwrite = TRUE)
...
![](ms_figures/fig_SI_map_mean_trend_rr_gp.png)

``` {r n_trend}

map_plot_file <- file.path(dir_git, 'ms_figures/fig_SI_map_sr_trend_rr_gp.png')

if(!file.exists(map_plot_file) | reload == TRUE) {
  
  n_trend_rast <- subs(rast_base, cell_summary, by = 'cell_id', which = 'sr_rr_trend')
  writeRaster(n_trend_rast, file.path(dir_git, 'output', 'sr_rr_trend_raster_gp.tif'),
              overwrite = TRUE)
...
![](ms_figures/fig_SI_map_sr_trend_rr_gp.png)
