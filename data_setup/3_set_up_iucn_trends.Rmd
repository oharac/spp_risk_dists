...
This process is accomplished in `1_set_up_iucn_risk.Rmd` and `2_set_up_iucn_risk_regional.Rmd`.  Resulting timeseries of historical assessments are available in `data/iucn/iucn_risk_timeseries_2017-3.csv` and `data/iucn/iucn_risk_ts_rgn_2017-3.csv` (or with updated version numbers as applicable).
...
if(!file.exists(trends_file)) {
  ...
  spp_risk_file <- file.path(dir_data, 'iucn',
  ...
  for(i in 1:nrow(spp_ids_valid)) { # i <- 1
    ...
    cat(i, 'of', nrow(spp_ids_valid), ': Trying spp', spp_id, 'from', rgn_id, '\n')
...
spp_risk_ts_file <- file.path(dir_data, 'iucn',
                              sprintf('iucn_risk_timeseries_%s.csv', api_version))
spp_risk_rgn_ts_file <- file.path(dir_data, 'iucn',
