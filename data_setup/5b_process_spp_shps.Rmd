...
dir_shp2 <- file.path(dir_M, sprintf('git-annex/globalprep/_raw_data/iucn_spp/d%s', api_version))
  ### These are shapefiles directly from IUCN as individual species map files
  ### (in the unzipped folder).
  ### in the base folder are shapefiles at a taxonomic level.
...
if(nrow(maps_to_rasterize) == 0) { ### all maps accounted for as .csvs
  ...
  for(i in seq_along(shps)) {
    ...
    polys_match <- polys_all %>%
      select(shp_iucn_sid = iucn_sid, sciname = binomial, subpop, presence, geometry) %>%
      mutate(subpop = as.character(subpop)) %>%
    ...
    # system.time({
      tmp <- parallel::mclapply(seq_along(spp_ids),
        ...
        ### AAAARRRGGHHH with this problem
        problem_spp <- c(21132910, 190317, 15598596, 15598672, 190197)
        if(spp %in% problem_spp) {
          cat_msg('Species ', spp, ' is a problem... skipping!')
          return(NA)
        }
        ...
        spp_shp <- valid_check(spp_shp)
          ### if invalid geometry, buffer to 0 and cast back to multipolygon
      
        spp_shp <- spp_shp %>%
          clip_to_globe() %>%
            ### in common_fxn.R: ensures no coordinates outside +-180, +-90
            ### otherwise transform to eq area proj creates horizontal lines
        ...
        if(nrow(spp_present) == 0) {
          msg_cat('Species ID ', spp, ' resulted in a zero-length dataframe.')
        }
