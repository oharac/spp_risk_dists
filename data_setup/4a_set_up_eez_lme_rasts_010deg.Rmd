...
if(!file.exists(eez_rast_file)) {
  ...
  plot(eez_rast_ocean, main = 'EEZ raster 0.10 deg lat-long')
...
if(!file.exists(bathy_rast_file)) {
  ...
  values(rast_neritic)[values(bathy_rast_raw) <= 200] <- 1
  values(rast_shallow)[values(bathy_rast_raw) <=  60] <- 1
  
  writeRaster(rast_neritic, file.path(dir_git, 'spatial', 'bathy_rast_neritic_010_wgs84.tif'))
  writeRaster(rast_shallow, file.path(dir_git, 'spatial', 'bathy_rast_shallow_010_wgs84.tif'))
...
land <- read_sf(eez_shp_file) %>%
  filter(str_detect(rgn_name, 'United States|Mexico') & str_detect(rgn_type, 'land')) %>%
  st_transform(4326) %>%
  fasterize::fasterize(raster(extent(c(-122, -116, 32, 34.5)), res = 0.02)) %>%
  rasterToPoints() %>% as.data.frame()

y <- bathy_rast %>%
  crop(extent(c(-122, -116, 32, 34.5))) %>%
  rasterToPoints() %>%
  as.data.frame() %>%
...
ggplot(y, aes(x, y)) +
  ...
  geom_raster(data = land, fill = 'grey20') +
  coord_cartesian(xlim = c(-122, -116), ylim = c(32, 34.5)) +
  scale_fill_distiller(palette = 'Blues', direction = 1) +
  labs(title = 'bathymetry, SB Channel Islands')
...
ggplot(x, aes(x, y)) +
  ...
  coord_cartesian(xlim = c(-122, -116), ylim = c(32, 34.5)) +
  scale_fill_distiller(palette = 'Blues', direction = 1) +
  labs(title = 'bathymetry categories: <=60m = 1, <=200 m = 2, >200m = 3')
