...
if(!file.exists(lat_rast_file)) {
  ...
  loiczid_lats <- read_csv(file.path(dir_o_anx, 'am_files', 'am_cells.csv')) %>%
    select(loiczid, lat = centerlat, long = centerlong)
  write_csv(loiczid_lats, file.path(dir_git, 'data/latlong_lookup.csv'))
...
## Create Longhurst raster

to WGS84, just to match LOICZID to Longhurst zones
``` {r create_Longhurst_rast}

longh_to_loiczid_file  <- file.path(dir_git, 'data/longhurst_cells.csv')

if(!file.exists(longh_to_loiczid_file)) {
  
  longhurst_sf <- st_read(file.path(dir_spatial, 'longhurst_v4_2010', 
                                    'Longhurst_world_v4_2010.shp')) %>%
    mutate(id = 1:nrow(.))
  ### already in WGS84 lat long
  
  loiczid_rast_wgs84 <- raster(file.path(dir_spatial, 'loiczid_raster.tif'))
  longhurst_rast <- fasterize::fasterize(longhurst_sf, loiczid_rast_wgs84,
                                         field = 'id')
  
  ### Interpolate using nearest neighbor
  library(gstat)

  xy <- data.frame(xyFromCell(longhurst_rast, 1:ncell(longhurst_rast))) 
    ### get xy coords into dataframe
  v  <- getValues(longhurst_rast)                                                  
    ### get cell values 
  tmpdf <- cbind(xy, v) %>% 
    filter(!is.na(v))                           
    ### create dataframe of x,y, and values; remove NAs (throws error since these are cells we are interpolating over)
  mg <- gstat(id = "v", formula = v ~ 1, 
              locations = ~ x + y, data = tmpdf,
              nmax = 7, set = list(idp = 0))
  
  longh_interp <- interpolate(longhurst_rast, mg) #, progress = 'text')
    ### interpolate across NA cells

  longhurst_to_loiczid <- raster::crosstab(longh_interp, loiczid_rast_wgs84) %>%
    as.data.frame() %>%
    setNames(c('longhurst', 'loiczid', 'freq')) %>%
    filter(freq > 0 & !is.na(longhurst)) %>%
    mutate(longhurst = as.double(longhurst),
           loiczid   = as.double(loiczid)) %>%
    select(-freq) %>%
    left_join(longhurst_sf %>% 
                as.data.frame() %>% select(id, code = ProvCode, desc = ProvDescr),
              by = c('longhurst' = 'id'))
  
  write_csv(longhurst_to_loiczid, longh_to_loiczid_file)
}
...
to WGS84, to match LOICZID to EEZ presence/absence
``` {r create_eez_rast}

eez_to_loiczid_file  <- file.path(dir_git, 'data/eez_cells.csv')

if(!file.exists(eez_to_loiczid_file)) {
  
  eez_sf <- st_read(file.path(dir_M, 'git-annex/globalprep/spatial', 
                                    'v2017/regions_2017_update.shp'))
  ### in Mollweide... :/
  eez_wgs84 <- eez_sf %>% 
    st_transform(4326) %>%
  ...
  # ggplot(eez_wgs84) +
  #   geom_sf(aes(color = rgn_id))

  
  loiczid_rast_wgs84 <- raster(file.path(dir_spatial, 'loiczid_raster.tif'))
  eez_rast <- fasterize::fasterize(eez_wgs84, loiczid_rast_wgs84,
                                         field = 'rgn_id')
  
  eez_rast <- eez_rast / eez_rast

  eez_to_loiczid <- raster::crosstab(eez_rast, loiczid_rast_wgs84) %>%
    as.data.frame() %>%
    setNames(c('eez', 'loiczid', 'freq')) %>%
    filter(freq > 0 & !is.na(eez)) %>%
    mutate(eez = as.double(eez),
           loiczid   = as.double(loiczid)) %>%
    select(-freq)
  
  write_csv(eez_to_loiczid, eez_to_loiczid_file)
}
...
IS THIS NOW OBSOLETE?  Instead, sample from the raster stack?
