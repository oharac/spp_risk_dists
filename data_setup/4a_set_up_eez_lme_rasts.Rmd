...
Using [General Bathymetric Chart of the Oceans (GEBCO) bathymetry raster](https://www.bodc.ac.uk/data/hosted_data_systems/gebco_gridded_bathymetry_data/), identify min depth of ocean cells.  Leaving as min depth allows for changing depth thresholds if desired.
...
if(!file.exists(bathy_rast_file)) {
  ...
  bathy_rast <- bathy_rast_min %>%
    raster::resample(rast_base_tenthdeg, method = 'ngb')
  
  values(bathy_rast)[is.na(values(eez_rast))] <- NA
  values(bathy_rast)[values(bathy_rast) < 0] <- 0
...
y <- bathy_rast %>%
  crop(extent(c(-165, -150, 17, 25)))
plot(y, main = 'bathymetry, Hawaii')

x <- bathy_rast_cats %>%
  crop(extent(c(-165, -150, 17, 25)))
plot(x, main = 'bathymetry categories, Hawaii')
