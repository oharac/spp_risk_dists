...
lme_rast_file <- file.path(dir_git, 'spatial', 'lme_rast.tif')
lme_shp_file  <- file.path(dir_o_anx, 'lme/LME66', 'LMEs66.shp')

if(!file.exists(lme_rast_file)) {
  lme_sf <- read_sf(lme_shp_file) %>%
    clip_to_globe()
    ### in common_fxn.R: ensures no coordinates outside +-180, +-90
  
  
  lme_sf_reclass <- lme_sf %>%
  ...
  lme_rast <- fasterize::fasterize(lme_sf_reclass, rast_base, 
