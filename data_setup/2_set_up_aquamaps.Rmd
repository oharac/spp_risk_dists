...
Also sets up data for sub-analyses:

* Species range, calculated at 0% presence threshold, 60% threshold, and probability-weighted area
* Taxonomic breakdown
...
am_spp_cells_file <- file.path(dir_o_anx, 'am_files', 'am_spp_cells_assessed.csv')
am_spp_cells_all_file <- file.path(dir_o_anx, 'am_files', 'am_spp_cells_all.csv')

### we'll save the entire file as well as a subset with just the assessed species
am_with_scores <- read_csv(file.path(dir_setup, 'int/am_with_scores.csv'))


if(!file.exists(am_spp_cells_file) | !file.exists(am_spp_cells_all_file)) {
  library(data.table)

  am_cells <- data.table::fread(file.path(dir_am_data, 'csv', 'hcaf_v6.csv'),
                                select = c('CsquareCode', 'LOICZID')) %>%
    clean_df_names() %>%
    distinct()
  ...
  asc_loiczid <- am_cells[am_spp_cells, on = 'csquarecode'] %>%
    select(am_sid = speciesid, loiczid, prob = probability)
  
  ### filtered to assessed spp only
  am_spp_cells_assessed <- asc_loiczid %>%
    filter(am_sid %in% am_with_scores$am_sid) 

  write_csv(am_spp_cells_assessed, am_spp_cells_file)
  write_csv(asc_loiczid, am_spp_cells_all_file)
  ...
  git_prov(am_spp_cells_all_file, filetype = 'output')
...
These will be calculated using the entire AquaMaps dataset, not just the IUCN assessed species.

``` {r spp_range}

am_spp_range_summary_file <- file.path(dir_git, 'data', 'am_spp_range_summary.csv')

if(!file.exists(am_spp_range_summary_file)) {
  
  am_cells <- data.table::fread(file.path(dir_am_data, 'csv', 'hcaf_v6.csv'),
                                select = c('LOICZID', 'OceanArea')) %>%
    clean_df_names() %>%
    distinct()
  
  ### Use the all-cells file modified to include LOICZID instead of CSquareCode
  am_spp_cells_all_file <- file.path(dir_o_anx, 'am_files', 'am_spp_cells_all.csv')
  
  am_spp_cells_areas <- data.table::fread(am_spp_cells_all_file) %>%
  ...
  write_csv(am_range_summary, am_spp_range_summary_file)

} else {
  am_range_summary <- read_csv(am_spp_range_summary_file)
}
...
am_range_summary1 <- read_csv(file.path(dir_git, 'data', 'am_spp_range_summary.csv')) %>%
  gather(pct_cut, area_km2, contains('pct')) %>%
  mutate(pct_cut = ifelse(pct_cut == 'area_km2_0pct', '0% threshold', '60% threshold'),
         area_km2 = area_km2 / 1e6,
         area_km2_probwt = area_km2_probwt / 1e6)

ggplot(am_range_summary1, aes(x = area_km2_probwt, y = area_km2, color = pct_cut)) +
  ggtheme_plot() +
  geom_abline(slope = 1, intercept = 0, color = 'blue4') +
  geom_point(alpha = .3) +
  labs(x = 'Area weighted by probability (million km^2)',
       y = 'Area clipped by threshold (million km^2)',
       color = 'Threshold')
