...
if(!file.exists(spp_range_file) | reload == TRUE) {
  ...
  if(reload == TRUE & file.exists(spp_range_file)) {
    spp_range_list_done <- read_csv(spp_range_file)
...
This will *over*predict range for many species whose range is limited by depth, due to polyon buffering.
...
poly_range_df <- lapply(spp_shps,
                        FUN = function(x) {
                          ### x <- spp_shps[1]
                          shp <- st_read(x)
                          shp <- shp %>%
                            filter(id_no %in% spp_maps$iucn_sid)
...
### make a dataframe of species risk, trend, regional risk, and range area
...
for(taxon in taxa) {
  ...
  if(!file.exists(taxon_risk_sum_file) | reload == TRUE) {
    ...
    cat('Processing ', length(taxon_ids), ' species maps in ', taxon, '...')
