---
title: 'Rasterize EEZs, LMEs, and Bathymetry - Gall-Peters'
...
Set up analysis rasters for EEZs, LMEs, and bathymetry at Gall-Peters 10 km^2 resolution.
...
eez_rast_file <- file.path(dir_git, 'spatial', 'eez_rast_gp.tif')
eez_shp_file  <- file.path(dir_M, 'git-annex/globalprep/spatial/v2017/regions_2017_update.shp')

area_rast <- raster(file.path(dir_git, 'spatial', 'ocean_area_rast_gp.tif'))
...
if(!file.exists(eez_rast_file)) {
  ...
  rast_base_gp <- area_rast
  
  eez_rast <- fasterize::fasterize(eez_shp_reclass, rast_base_gp, 
...
Using the LME shapefile, we will include all MLEs and then mask out land using the area raster (for consistency).

``` {r set up LME raster}

lme_rast_file <- file.path(dir_git, 'spatial', 'lme_rast_gp.tif')
...
if(!file.exists(lme_rast_file)) {
  ...
  lme_rast <- fasterize::fasterize(lme_shp_reclass, rast_base_gp, 
  ...
  lme_rast <- raster::raster(lme_rast_file)
}

plot(lme_rast, main = 'LME raster 10 km^2 Gall-Peters')
...
bathy_rast_file <- file.path(dir_git, 'spatial', 'bathy_rast_gp.tif')
...
if(!file.exists(bathy_rast_file)) {
  ...
  writeRaster(rast_neritic, file.path(dir_git, 'spatial', 'bathy_rast_neritic_gp.tif'),
              overwrite = TRUE)
  writeRaster(rast_shallow, file.path(dir_git, 'spatial', 'bathy_rast_shallow_gp.tif'),
              overwrite = TRUE)
