...
dir_shp <- file.path(dir_M, sprintf('git-annex/globalprep/_raw_data/iucn_spp/d%s', '2017-3'))
...
shps_df <- lapply(shps_dbfs, FUN = function(x) foreign::read.dbf(x, as.is = TRUE)) %>%
...
map_info_list <- lapply(map_files, FUN = function(x) {
...
if(nrow(maps_to_rasterize) == 0) { ### all maps accounted for as .csvs
  ...
  for(i in seq_along(shps)) {
    ...
    cat_msg(i, ' of ', length(shps), ': reading ', shp, ' from: \n  ', shp_full)
    ...
    cat_msg(i, ' of ', length(shps), ': Processing ', shp, ' with ', length(spp_ids), ' species...')
    ...
    # system.time({
      tmp <- parallel::mclapply(seq_along(spp_ids),
        ...
        ### x <- 1
        spp <- spp_ids[x]
        cat_msg(x, ' of ', length(spp_ids), ': Processing ', spp, ' in ', shp,
                ' (group ', i, ' of ', length(shps), ')...\n')
        
        spp_shp <- polys_match %>%
          filter(iucn_sid == spp)
        
        valid_check <- st_is_valid(spp_shp)
          ### can return a vector if multiple polygons with same ID
        if(any(!valid_check)) {
          cat_msg('Found invalid geometries in ', spp, '; fixing with buffer = 0...')
          
          ### check area before and after buffering to make sure no loss
          area_pre  <- st_area(spp_shp)
          spp_shp   <- st_buffer(spp_shp, dist = 0)
          area_post <- st_area(spp_shp)
          
          ### error check to make sure the buffer didn't lose polygons
          if(!all.equal(area_pre, area_post)) {
              ### use all.equal() for near equality, and for comparing all 
              ### elements in case of a vector
            cat_msg('Error: iucn_sid: ', spp, ': area_pre = ', area_pre, 
                    '; area_post = ', area_post, '; not equal!')
            stop('Area_pre and area_post not equal!')
          } else {
            cat_msg('Area check good! iucn_sid: ', spp, ': area_pre = ', area_pre, 
                    '; area_post = ', area_post, '; all equal!')
          }
        }
