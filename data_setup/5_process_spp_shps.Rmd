...
if(nrow(maps_to_rasterize) == 0) { ### all maps accounted for as .csvs
  ...
  if(any(maps_to_rasterize$max_depth != '200 m +')) { 
    message('generating neritic and shallow rasters')
  ...
  for(i in seq_along(shps)) {
    ...
    polys <- read_sf(shp_full) %>%
      mutate(subpop = ifelse(!'subpop' %in% names(.), NA_character_, subpop)) %>% 
        ### BOTW doesn't have subpop column
    ...
    # system.time({
      tmp <- parallel::mclapply(seq_along(spp_ids),
        ...
        write_csv(spp_present, file.path(dir_o_anx, 'spp_rasters',
                                         sprintf('iucn_sid_%s_010deg.csv', spp)))
        return(NA)
