...
spp_sys_from_api_file <- file.path(dir_o_anx, sprintf('iucn/spp_systems_from_api_%s.csv', api_version))
reload <- FALSE
...
marg_suit <- spp_marine %>%
  ...
  summarize(suit_all = tolower(paste0(unique(suitability), collapse = ', ')),
            intertidal_only = sum(!cat %in% c(12) & !(cat == 9 & subcat1 == 10)) == 0) %>%
              ### cat 9.10 is estuaries - if only intertidal or estuaries, check for suitability
