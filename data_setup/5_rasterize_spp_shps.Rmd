...
Using a set of IUCN species range maps, rasterize each species to 0.1° lat long raster using `fasterize`.  Use `presence` field from shapefile.
...
map_info_sans_dupes <- map_info_raw %>%
  ...
  filter((length(unique(dbf_file)) == 1) | (!str_detect(dbf_file, 'MARINEFISH|REPTILE|CORALS_PART_2_1|MANGROVES'))) %>%
...
marine_spp_ids <- read_csv(file.path(dir_git, 'data/iucn',
...
* Loop over each `iucn_sid` in the shapefile, rasterizing (`fasterize()`) to 0.10 degree resolution, using "presence" field. 
    * clip to neritic (<=200 m) and shallow (<=60 m) depth raster if appropriate
...
maps_to_rasterize <- read_csv(file.path(dir_git, 'data/iucn',
                                        sprintf('spp_marine_maps_%s.csv', api_version)),
                              col_types = 'ddciccc') %>%
...
if(reload == FALSE) {
  maps_already_rasterized <- list.files(file.path(dir_o_anx, 'spp_rasters'),
                                        pattern = '010deg.csv') %>%
    str_replace_all('iucn_sid_|_010deg.csv', '') %>%
    as.integer()
...
if(nrow(maps_to_rasterize) == 0) { ### all maps accounted for as .csvs
  ...
  rast_base <- raster(resolution = .10, ext = extent(-180, 180, -90, 90)) %>%
    setValues(1:length(.))
  
  ### rast_bathy for depth clipping: create neritic mask and shallow mask from blank.
  ### Do this only if there are any species with limited max depths...
  if(any(maps_to_rasterize$max_depth != '200 m +')) { 
    rast_bathy <- raster(file.path(dir_git, 'spatial',
                                   'bathy_rast_010_wgs84.tif'))
    rast_neritic <- rast_shallow <- raster(resolution = .10, ext = extent(-180, 180, -90, 90))
    values(rast_neritic)[values(rast_bathy) <= 200] <- 1
    values(rast_shallow)[values(rast_bathy) <=  60] <- 1
  }
  ...
  for(i in seq_along(shps)) {
    ...
    polys <- read_sf(file.path(dir_shp, shp)) %>%
    ...
    # system.time({
      tmp <- parallel::mclapply(seq_along(spp_ids),
        ...
        ### depth clip if necessary
        if(spp_shp$max_depth == '< 20 m') {
          spp_rast <- mask(spp_rast, rast_shallow)
        }
        if(spp_shp$max_depth == '< 200 m') {
          spp_rast <- mask(spp_rast, rast_neritic)
        }
        ### write out as a raster:
        # rast_file <- file.path(dir_o_anx, sprintf('spp_rasters/iucn_sid_%s_010deg.tif', spp))
        ...
        spp_present <- data.frame(cell_id  = 1:length(rast_base),
                                  presence = values(spp_rast)) %>%
          filter(!is.na(presence))
        write_csv(spp_present, file.path(dir_o_anx, sprintf('spp_rasters/iucn_sid_%s_010deg.csv', spp)))
        return(NA)
...
maps <- read_csv(file.path(dir_git, 'data/iucn',
                           sprintf('spp_marine_maps_%s.csv', api_version)),
                 col_types = 'ddciccc') %>%
