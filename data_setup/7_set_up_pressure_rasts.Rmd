...
For the purposes of this investigation, we will use the pressures which include stress and habitat vulnerability.
...
for (layer in prs_layers) { ### layer <- prs_layers[1]
  lyr_rast_raw <- raster(layer)
  lyr_rast <- mask(lyr_rast_raw, ocean_base)
  
  lyr_distr_file <- sprintf('prs_distr_plots/%s_dist.png', names(lyr_rast))
