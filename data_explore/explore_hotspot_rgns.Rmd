...
spp_risk_all <- spp_risk %>%
  ...
  left_join(spp_ranges, by = c('iucn_sid', 'sciname')) %>%
  select(iucn_sid, iucn_rgn, cat_score, range_km2)

cells_info <- read_csv(cell_summary_file) %>%
  select(cell_id, cell_group, n_spp, lme_id, depth) %>%
  distinct()
...
taxa_cells_info <- read_csv(taxa_cells_file, col_types = 'ddc') %>%
...
spp_cells_info <- taxa_cells_info %>%
  select(cell_id, iucn_sid, spp_gp, iucn_rgn, 
         cat_score, range_km2, cell_group, depth)
...
``` {r create_rgn_maps_w_points, results = 'asis'}


for(cell_gp in unique(spp_cells_info$cell_group)) {
  ### cell_gp <- spp_cells_info$cell_group[1]
  ...
  spp_cell_gp <- spp_cells_info %>%
    filter(cell_group == cell_gp) %>%
    left_join(cells_latlong, by = 'cell_id') %>%
    select(-x, -y, -cell_group)
  ...
  cat(sprintf('![](%s)\n', path.expand(fig_file)))
  
  DT::datatable(spp_cell_gp, caption = cell_gp) %>% print()
}
```
