...
for(stressor_file in cc_stressor_files) {  ### stressor_file <- cc_stressor_files[3]
  ...
  tmp <- read_csv(stressor_file, col_types = 'ddddd') %>%
    ...
    mutate(stressor_mean = ifelse(is.nan(stressor_mean) & stressor_name != 'uv', 
                                  0, stressor_mean)) %>%
    setNames(c('loiczid', 
...
risk_df <- read_csv(file.path(dir_data, 'risk_by_cell_all.csv'),
                    col_types = 'ddddd') %>%
  select(loiczid, n_spp, mean_risk)
...
### Create matrices and vectors to make sure they're in appropriate cluster order
...
Recalculate with adjusted clusters; this should join ecoregions that are too small and divide ecoregions that are too large.  Let's try with 150 clusters.
