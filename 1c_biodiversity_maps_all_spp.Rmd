...
if(!file.exists(cell_summary_file) | reload == TRUE) {
  ...
  ptm <- system.time({
    cell_vals_list <- parallel::mclapply(1:n_chunks, mc.cores = 6,
        FUN = function(x) { ### x <- 1
...
land_poly <- sf::read_sf(file.path(dir_spatial, 
                                   'ne_10m_land/ne_10m_land.shp')) %>%
  st_transform(gp_proj4)


map_rast_file <- file.path(dir_output, 'mean_risk_raster_all.tif')
...
if(!file.exists(map_rast_file) | reload == TRUE) {
  ...
  ### mean_rast <- raster(file.path(dir_output, 'mean_risk_raster_all.tif'))
...
map_rast_file <- file.path(dir_output, 'var_risk_raster_all.tif')
...
map_rast_file <- file.path(dir_output, 'n_spp_risk_raster_all.tif')
...
map_rast_file <- file.path(dir_output, 'n_threat_raster_all.tif')
...
if(!file.exists(map_rast_file) | reload == TRUE) {
  ...
  writeRaster(n_threat_rast, map_rast_file,
              overwrite = TRUE)
  ### n_threat_rast <- raster(file.path(dir_output, 'n_threat_raster_all.tif'))

  pct_threat_rast <- subs(rast_base, cell_summary, by = 'cell_id', which = 'pct_threatened')
  writeRaster(pct_threat_rast, file.path(dir_output, 'pct_threat_raster_all.tif'),
              overwrite = TRUE)
  ### n_threat_rast <- raster(file.path(dir_output, 'n_threat_raster_all.tif'))
...
map_rast_file <- file.path(dir_output, 'trend_raster_all.tif')
...
map_rast_file <- file.path(dir_output, 'n_trend_raster_all.tif')
