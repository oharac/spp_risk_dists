...
set.seed(5132015)
kf <- kfold(nrow(dta))
  ### assign cluster numbers to the length of a dataset; default k = 5

rmse <- rep(NA, 5)
for (k in 1:5) {
  ### iterate over each value of k, alternating test and others as train
  test <- dta[kf == k, ]
  train <- dta[kf != k, ]
  v <- dismo::voronoi(train)
...
``` {r}
gs <- gstat(formula=prec~1, locations=dta) ### no idp=0 (distance unweighted) and no nmax=5 (use all pts?)
...
``` {r}
x <- read.csv("http://rspatial.org/analysis/data/airqual.csv")
...
for (i in 1:nfolds) {
  test        <- aq[k != i, ]
  train       <- aq[k == i, ]
  m           <- gstat(formula = OZDLYAV ~ 1, locations = train, nmax = opt$par[1], 
