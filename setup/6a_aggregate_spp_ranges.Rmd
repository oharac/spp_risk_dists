...
meow_rgns_rast <- raster::raster(file.path(dir_git, 'spatial', 'meow_rast.tif'))
meow_cells_all <- data.frame(cell_id = values(rast_cell_ids),
                             meow_id = values(meow_rgns_rast)) %>%
  filter(!is.na(meow_id))
meow_to_rgn <- read_csv(file.path(dir_git, 'spatial/iucn_rgn_to_meow.csv'))
...
for(taxon in taxa) {
  ...
  taxon_sum_file <- file.path(dir_o_anx, 'taxa_summaries',
                                   sprintf('%s_cell_sum_noncomp_%s.csv', tolower(taxon), api_version))
