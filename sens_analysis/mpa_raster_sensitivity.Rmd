...
mpa_poly_small <- mpa_poly_df %>%
  mutate(`MPA size` = case_when(area_km2 < 1 ~ '< 1 km²',
                          ...
                          TRUE ~ '>= 1000 km²')) %>%
  group_by(`MPA size`) %>%
  summarize(`A total, km²` = sum(area_km2) %>% round(3),
            N = n()) %>%
  mutate(`% MPA by count` = (N / sum(N) * 100) %>% round(3),
...
pct_real_diff_all_df <- d_area_all_df %>%
  ...
  mutate(res = paste0('Sum diff (', res, ')'),
...
```{r create cell ID rasts at different resolutions, eval = FALSE}
# cell_id <- raster(file.path(dir_spatial, 'cell_id_rast.tif'))
# cell_id_3km_file  <- file.path(dir_o_anx, 'spatial/cell_id_3km.tif')
# notake_3km  <- raster(file.path(dir_sa, 'notake_no_intsx_rast_by_id_3km.tif'))
# 
# rast_cell_id <- resample(cell_id, notake_3km, 
#                          method = 'ngb', filename = cell_id_3km_file,
...
notake_2km  <- raster(file.path(dir_sa, 'notake_no_intsx_rast_by_id_2km.tif'))
notake_3km  <- raster(file.path(dir_sa, 'notake_no_intsx_rast_by_id_3km.tif'))
...
cell_id_2km  <- raster(file.path(dir_o_anx, 'spatial/cell_id_2km.tif'))
cell_id_3km  <- raster(file.path(dir_o_anx, 'spatial/cell_id_3km.tif'))
...
cell_area_file_2km  <- file.path(dir_sa, 'notake_area_2km.csv')
cell_area_file_3km  <- file.path(dir_sa, 'notake_area_3km.csv')
cell_area_file_5km  <- file.path(dir_sa, 'notake_area_5km.csv')

if(!file.exists(cell_area_file_5km)) {
  df <- calc_rast_to_10km(notake_5km, cell_id_5km, 
                          cell_area_file_5km, mc.cores = 15)
}
if(!file.exists(cell_area_file_3km)) {
  df <- calc_rast_to_10km(notake_3km, cell_id_3km, 
                          cell_area_file_3km, mc.cores = 15)
}
if(!file.exists(cell_area_file_2km)) {
  df <- calc_rast_to_10km(notake_2km, cell_id_2km, 
                          cell_area_file_2km, mc.cores = 15)
}
if(!file.exists(cell_area_file_1km)) {
  df <- calc_rast_to_10km(notake_1km, cell_id_1km, 
                          cell_area_file_1km)
}
if(!file.exists(cell_area_file_500m)) {
  df <- calc_rast_to_10km(notake_500m, cell_id_500m, 
                          cell_area_file_500m)
}
...
area_1km <- read_csv(cell_area_file_1km) %>%
  summarize(area_rast_10km = sum(prot_area_km2),
            res = '1 km')
area_2km <- read_csv(cell_area_file_2km) %>%
  summarize(area_rast_10km = sum(prot_area_km2),
            res = '2 km')
area_3km <- read_csv(cell_area_file_3km) %>%
  summarize(area_rast_10km = sum(prot_area_km2),
            res = '3 km')
...
calc_areas <- notake_tot_areas_df %>%
  left_join(bind_rows(area_500m, area_1km, area_2km, area_3km, area_5km),
            by = 'res') %>%
  mutate(`Information loss = A(raster, orig) - A(raster, 10km)` = area_rast - area_rast_10km) %>%
  rename(`Original res` = res,
         `Area (polygon)` = area_poly,
         `Area (raster, orig)` = area_rast,
         `Area (raster, 10km)` = area_rast_10km)
