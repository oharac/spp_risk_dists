...
knitr::kable(beta_se_df, format = 'latex', caption = 'Table i: Standard errors and t statistics',
...
knitr::kable(r2_df, format = 'latex', caption = 'Table iii: Measures of fit',
...
knitr::kable(results_df, format = 'latex',
  ...
  add_header_above(c(' ' = 2,
                     ...
                     '$\\mathbf{\\widehat V_{\\hat\\beta_{clust}}}$' = 2,
                     '$\\mathbf{\\widetilde V_{\\hat\\beta_{alt.clust}}}$' = 2))

```

Basing significance on prediction errors calculated from the alternative cluster-robust covariance matrix estimator $\mathbf{\widetilde V_{\hat\beta_{alt.clust}}}$, it appears that the dummy for south latitudes, $\delta_{\lambda}$, as well as the interaction terms $\delta_{\lambda}x_{UV}$, $\delta_{\lambda}x_{SST}$, and $\delta_{\lambda}x_{OA}$, are not statistically significant ($|t| > 1.96$).  In addition, $x_{UV}$ just misses the critical value for $p = .05$.  
