...
comp_assessed_taxa <- read_csv(file.path(dir_data, 
                                         sprintf('spp_marine_maps_%s.csv', api_version)),
                               col_types = 'ddcicccl') %>%
  select(dbf_file, comp_assessed) %>%
...
if(!file.exists(cell_summary_file) | reload == TRUE) {
  ...
  sum_files <- list.files(dir_taxa_summaries,
                          ...
                          full.names = TRUE)
  comp_files <- sum_files[basename(sum_files) %>% 
                            str_replace_all('_part.*|_cell_sum.*', '') %in% comp_assessed_taxa]
  ...
  ptm <- system.time({
    cell_values_all <- parallel::mclapply(comp_files, mc.cores = 24,
                                          FUN = function(x) { ### x <- comp_files[3]
                                            y <- read_csv(x, col_types = 'dddddddddi')
                                          }) %>%
      setNames(basename(comp_files)) %>%
...
map_rast_file <- file.path(dir_git, 'output', 'mean_rr_risk_raster_comp.tif')
...
if(!file.exists(map_rast_file) | reload == TRUE) {
  ...
  ### mean_rast <- raster(file.path(dir_git, 'output', 'mean_rr_risk_raster_comp.tif'))
...
map_rast_file <- file.path(dir_git, 'output', 'var_rr_risk_raster_comp.tif')
...
map_rast_file <- file.path(dir_git, 'output', 'sr_rr_risk_raster_comp.tif')
...
map_rast_file <- file.path(dir_git, 'output', 'sr_rr_threat_raster_comp.tif')
...
if(!file.exists(map_rast_file) | reload == TRUE) {
  ...
  writeRaster(n_threat_rast, map_rast_file,
              overwrite = TRUE)
  ### n_threat_rast <- raster(file.path(dir_git, 'output', 'sr_rr_threat_raster_comp.tif'))
  ...
  writeRaster(pct_threat_rast, file.path(dir_git, 'output', 'sr_rr_pct_threat_raster_comp.tif'),
              overwrite = TRUE)
  ### n_threat_rast <- raster(file.path(dir_git, 'output', 'sr_rr_pct_threat_raster_comp.tif'))
...
map_rast_file <- file.path(dir_git, 'output', 'trend_rr_raster_comp.tif')
...
map_rast_file <- file.path(dir_git, 'output', 'sr_rr_trend_raster_comp.tif')
