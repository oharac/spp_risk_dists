---
title: 'MPAs and biodiversity intactness - conservation-focused MPAs'
...
``` {r set up conservation mpa vs risk df}
...
mpa_recent_df <- read_csv(file.path(dir_git, '_spatial', 'wdpa_mpa_year.csv'),
                           col_types = 'dddd') %>%
...
``` {r conserv mpas, eval = TRUE}

global_conserv <- risk_mpa_df %>%
  filter(eez < 255 & eez != 213) %>%
  mutate(lbl = 'Global EEZ')

global_conserv_recent <- risk_mpa_recent_df %>%
  filter(eez < 255 & eez != 213) %>%
  mutate(lbl = 'Global EEZ post-Aichi')

highseas_conserv <- risk_mpa_df %>%
  filter(eez > 255 | eez == 213) %>%
  mutate(lbl = 'High seas')

rgn_conserv <- risk_mpa_df %>%
  filter(eez < 255 & eez != 213) %>%
  mutate(lbl = r1_label) %>%
  arrange(lbl)

conserv_df <- bind_rows(global_conserv, global_conserv_recent, 
                        rgn_conserv, highseas_conserv) %>%
...
``` {r set up conserv vs risk rr df}
...
global_conserv_rr <- risk_mpa_rr_df %>%
  filter(eez < 255 & eez != 213) %>%
  mutate(lbl = 'Global EEZ')

global_conserv_rr_recent <- risk_mpa_rr_recent_df %>%
  filter(eez < 255 & eez != 213) %>%
  mutate(lbl = 'Global EEZ post-Aichi')

highseas_conserv_rr <- risk_mpa_rr_df %>%
  filter(eez > 255 | eez == 213) %>%
  mutate(lbl = 'High seas')

rgn_conserv_rr <- risk_mpa_rr_df %>%
  filter(eez < 255 & eez != 213) %>%
  mutate(lbl = r1_label) %>%
  arrange(lbl)

conserv_rr_df <- bind_rows(global_conserv_rr, global_conserv_rr_recent, 
                           rgn_conserv_rr, highseas_conserv_rr) %>%
  mutate(lbl = forcats::fct_inorder(lbl))
...
mpa_plot_unweighted <- plot_dist(conserv_df, conserv_means, 'MPA I-IV')

mpa_plot_rrweighted <- plot_dist(conserv_rr_df, conserv_rr_means, 'MPA I-IV') +
...
ggsave(file.path(dir_git, 'ms_figures/fig_S2a_bd_risk_vs_mpas_i_iv.png'),
       height = 3, width = 4.75, dpi = 300, units = 'in')
```

![](ms_figures/fig_S2a_bd_risk_vs_mpas_i_iv.png)

### unweighted means

`r knitr::kable(conserv_means)`

### rr-weighted means

`r knitr::kable(conserv_rr_means)`

``` {r save means}
means_to_save <- conserv_means %>%
  left_join(conserv_rr_means %>% rename(mu_rr = mu),
            by = c('lbl', 'mpa_status'))
write_csv(means_to_save, file.path(dir_git, '_setup/int/mpa_mean_risks_i_iv.csv'))
```