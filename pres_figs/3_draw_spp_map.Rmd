...
for(rast in rasts) { ### rast <- rasts[1]
  spp_rast <- raster(rast)
  spp_png <- rast %>%
    str_replace('rast', 'fig') %>%
    str_replace('.tif', '.png')
  mean_map <- two_panel(spp_rast,
                        colors = risk_cols,
                        values = risk_vals,
                        labels = risk_lbls,
                        breaks = risk_brks)
  ggsave(spp_png, width = 7.25, height = 3.7, units = 'in', dpi = 300)
}

lb_rast <- raster(file.path(dir_git, 'pres_figs/rast/dermochelys_coriacea_risk.tif'))
### normalize and set all values to 0.4 (global = vulnerable)
lb_rast <- lb_rast/lb_rast * 0.4

lb_global_map <- two_panel(lb_rast,
                      colors = risk_cols,
                      values = risk_vals,
                      labels = risk_lbls,
                      breaks = risk_brks)
ggsave(file.path(dir_git, 'pres_figs/fig/dermochelys_coriacea_global.png'), 
