...
# write_csv(taxa_sums %>% select(shp_basename, taxon) %>% distinct(),
...
spp_maps <- read_csv(file.path(dir_git, 'data', sprintf('spp_marine_maps_%s.csv', api_version)),
                     col_types = 'ddciccc') %>%
  mutate(shp_basename = tolower(dbf_file) %>%
           basename() %>%
...
for(i in seq_along(taxa_gps)) { ### i <- 3
  ...
  taxa_rast_file <- file.path(dir_o_anx, 'taxa_rasters', 
                              paste0(tolower(taxon_txt), '_rr.tif'))
